...
export class ConversationsService {
    ...
    async addMessage(data: AddMessageDto) {
        // First, try to get the conversation by ID, then by session_id
        let conversation = null;
        
        // Try to find by conversation ID first (if it's a UUID)
        if (data.conversationId && data.conversationId.length > 10) {
            const { data: convById } = await this.supabase.client
                .from('conversations')
                .select('id')
        ...
        if (!conversation) {
            const { data: convBySession } = await this.supabase.client
                .from('conversations')
                .select('id')
                .eq('session_id', data.conversationId)
                .single();
            conversation = convBySession;
        }
